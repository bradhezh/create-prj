...
import { regType, meta, NPM, Conf, PluginType, Spinner } from "@/registry";
import {
  installTmplt,
  setPkgName,
  setPkgVers,
...
const run = (type: PluginType) => {
  return async (conf: Conf, s: Spinner) => {
    const npm = conf.npm;
...
const install = async (npm: NPM, key: string, cwd: string, s: Spinner) => {
  ...
  p.log.warn(format(message.noTmpltCmd, key));
};
...
const create = async (npm: NPM, command: string, cwd: string, s: Spinner) => {
  ...
  const git = path.join(cwd, gitDir);
  if (
    !(await access(git)
      .then(() => true)
      .catch(() => false))
